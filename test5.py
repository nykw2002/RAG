...
Extract the most relevant search terms for this query."""
    ...
    def generate_search_function(self, query: str) -> str:
        """Ask o3-mini to generate a focused, precise search function"""
        print(f"🤖 Analyzing query: '{query}'")
        
        system_prompt = """You are an expert Python programmer. Create a search function that identifies the PRIMARY SUBJECT and searches precisely for that.

CRITICAL INSTRUCTIONS FOR SEARCH LOGIC:
1. Identify the PRIMARY ENTITY/SUBJECT from the query (like "Israel", "CAPA", "batch", etc.)
2. Search ONLY for lines containing that primary entity
3. DO NOT search for generic terms like "complaint", "status", "item", "how many" - these are just question words
4. Use case-insensitive matching
5. Keep it simple - one main search term

EXAMPLES:
- "How many complaints for Israel?" → Search for "israel" ONLY (not "complaint")
- "What's the CAPA status?" → Search for "capa" ONLY (not "status") 
- "Find rejected batches" → Search for "reject" OR "rejected" ONLY (not "batch")
- "Show ongoing deviations" → Search for "deviation" ONLY (not "ongoing")

PATTERN TO FOLLOW:
1. Identify 1 key term from the query (the main subject)
2. Search for lines containing that term only
3. Return focused, precise matches
...
Focus on the PRIMARY SUBJECT and search for that ONLY. Ignore question words.
...
def search_function(document_text: str) -> List[Dict[str, Any]]:
    ...
    for i, line in enumerate(lines):
        ...
        # Search for PRIMARY SUBJECT only - ignore question words
        if 'PRIMARY_TERM_HERE' in line_lower:  # Replace with actual term
            results.append({{
                'line_number': i + 1,
                'content': line.strip(),
                'details': {{'search_method': 'ai_focused'}}
            }})
    
    return results"""
        
        try:
            response = self.query_ai(system_prompt, user_prompt, max_tokens=1000)
            ...
            print(f"✅ Generated focused search function")
            return code.strip()
    ...
    def search_single_query(self, document_text: str, query: str) -> Dict[str, Any]:
        """Focused search: AI generates precise search logic for exact results"""
        print(f"🔍 Focused search for: '{query}'")
        
        # Debug for Israel queries to verify data exists
        if 'israel' in query.lower():
            self.debug_israel_search(document_text)
        
        # Generate focused AI search function
        print("🤖 Generating focused search function...")
        ...
        if function_code:
            print("⚡ Executing focused search...")
            results = self.execute_search_function(function_code, document_text)
            if results:
                search_method = "AI-generated (focused)"
        
        # Fallback to simple search if AI generation fails
        if not results:
            print("🔄 AI generation failed, using simple search fallback...")
            results = self.simple_search(document_text, query)
            search_method = "Simple fallback"
        
        # For Israel queries, verify we got the expected count
        if 'israel' in query.lower():
            expected_count = 14
            if len(results) != expected_count:
                print(f"⚠️ Expected {expected_count} Israel results, got {len(results)}. Results may be incomplete.")
        
        print(f"✅ Found {len(results)} results")
        
        # Simple, direct analysis without token limits
